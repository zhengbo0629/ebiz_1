 ...
 */
@WebServlet("/OverseaBuyerRegistrationServlet")
public class OverseaBuyerRegistrationServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;      
    /**
     * @see HttpServlet#HttpServlet()
     */
    public OverseaBuyerRegistrationServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (info.size() != 0) {
			...
			request.getRequestDispatcher("overseabuyerregistration.jsp").forward(request, response);// 跳转
		} else {
			...
			EbizUser user=new EbizUser(0,username,firstName,lastName,companyName, password, "11111111", contactEmail, contactPhone,  address+","+zipcode,
					createTime,createTime,"","UnActive",EbizUserTypeEnum.Oversea_Buyer.getName(), 0,1000,"" );

			for(EbizUserPermissionEnum permission:EbizUserPermissionEnum.values()){
				if(permission.getRole().equals("OverseaBuyerDefault"))
