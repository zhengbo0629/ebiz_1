 ...
 */
@WebServlet("/TakeOneUserLabelTaskServlet")
public class TakeOneUserLabelTaskServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public TakeOneUserLabelTaskServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<EbizPackage> packages=EbizSql.getInstance().readNeedLabeledPackForUser(currentUser,currentCompany,pickedUserName);
		...
		for(int i=0;i<packages.size();i++){
			if (currentTaskNumber < checkTaskLimit) {
				boolean update = EbizSql.getInstance().takeLabelTasks(currentUser, currentCompany, packages.get(i).UID);
				if (update) {
