 ...
 */
@WebServlet("/TakePayTasksServlet")
public class TakePayTasksServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public TakePayTasksServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for(int i=0;i<UIDList.size();i++){
			if (currentTaskNumber < checkTaskLimit) {
				boolean update = EbizSql.getInstance().takePayTasks(currentUser, currentCompany, UIDList.get(i));
				if (update) {
