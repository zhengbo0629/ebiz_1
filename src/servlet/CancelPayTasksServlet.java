 ...
 */
@WebServlet("/CancelPayTasksServlet")
public class CancelPayTasksServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public CancelPayTasksServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for(int i=0;i<UIDList.size();i++){
				...
				boolean update = EbizSql.getInstance().cancelPayTask(currentUser, currentCompany, UIDList.get(i));
				if (update) {
