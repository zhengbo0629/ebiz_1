 ...
 */
@WebServlet("/TakeOneCheckTaskServlet")
public class TakeOneCheckTaskServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public TakeOneCheckTaskServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for(int i=0;i<UIDList.size();i++){
			if (currentTaskNumber < checkTaskLimit) {
				boolean update = EbizSql.getInstance().takeCheckTasks(currentUser, currentCompany, UIDList.get(i));
				if (update) {
