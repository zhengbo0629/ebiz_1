		...
		<div
			...
			<div style="width: 100%; overflow: auto;">
				...
				<%
					...
					List<EbizUser> allDoctorUsers=EbizSql.getInstance().readAllDoctorUsers(offset, pagesize);
				...
				<table style="table-layout:fixed;word-wrap: break-word;font-size: 11px">
					...
					<%
							if (allDoctorUsers != null) {
								for (int i = 0; i < allDoctorUsers.size(); i++) {
									EbizUser row = allDoctorUsers.get(i);
						%>
					<tr>
						<td><%=row.UID%></td>
						...
						<td  style="font-size: 10px"><%=row.createTime%><br><%=row.updateTime%></td>

						<td><%=row.getStatus()%></td>
						<td  style="font-size: 10px"><%=row.note%></td>
						<td><%=row.balance%></td>
						<td><form action="updateDoctorUserInfor.jsp" method="POST">
			...
			<div style="text-align: right; margin-right: 0px">
				<br>
				<h4>
					<a href='doctorUserManage.jsp?PageNumber=<%=prevpage%>'>PrevPage</a><a>
					</a><a href='doctorUserManage.jsp?PageNumber=<%=nextpage%>'>NextPage</a>
				</h4>
			</div>
