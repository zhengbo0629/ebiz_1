		...
		<div
			...
			<div style="width: 100%; overflow: auto;">
				...
				<%
					...
					List<EbizPackage> unReceivedPacages = EbizSql.getInstance().readUnReceivedAndUnConfirmedSellOrOBOPackagesForUser(offset, pagesize, user);
			...
			<div style="text-align: right; margin-right: 0px">
				<br>
				<h4>
					<a href='unReceivedPacAndUnConfirmed.jsp?PageNumber=<%=prevpage%>'>上一页</a><a>
					</a><a href='unReceivedPacAndUnConfirmed.jsp?PageNumber=<%=nextpage%>'>下一页</a>
				</h4>
			</div>
				<table>
					<col width=5%>
					...
					<tr>
						...
						<td><form name="deleteForm<%=row.UID%>" id="deleteForm<%=row.UID%>" action='DeletNursePackServlet'  method="post"> 
								<input type='hidden' id='deletPackage' name='deletPackage' value="<%=row.UID%>" /><input
									onclick="deletePack(this)" type='button' style='width: 50px; font-size: 13px' name='submit-btn'
									value='Delete' />
							</form></td>
					</tr>
					<%
		...
		$("#".concat(source.form.name)).ajaxSubmit({
			...
			success : function(data) {
				...
				if(data.includes("Sucessfully")){
					location.href = "unReceivedPacAndUnConfirmed.jsp";
