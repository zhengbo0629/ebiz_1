...
<body>
	...
	<div id="wrapper">
		<%
			...
			List<EbizPackage> shippedPackages = EbizSql.getInstance().readPayingTasksForCompany(user, company, offset,
					pagesize);
		%>
		<%
		...
		<div id="wrapperContent">
			<div align="center" style="padding-bottom: 1px; padding-top: 10px;">
				<h3 style="margin-top: 5px;">所有领取的支付任务(All Taken Paying
					Tasks)</h3>
			</div>
			...
			<div id="threeBox">
				<div class="alignleft">
					...
					<form name="checkboxForm" id="checkboxForm"
						action="CancelPayTasksServlet" method="post">
				...
				<div class="aligncenter">
					<form name="sendCheckboxForm" id="sendCheckboxForm"
						action="payPackages.jsp" method="post">
					<input onclick="payGroupTasks()" type="button" value="批量支付"
						name="updateAll"
						style="margin-top: 10px; width: 90px; font-size: 12px; height: 30px; background-color: #ffcebf">
			...
			<div style="width: 100%; overflow: auto;">
				...
				<table
					...
					<tr>
						...
						<td
							style='font-size: 12px; line-height: 15px; padding-top: 5px; padding-bottom: 5px'><%=row.creditcardNumber%></td>



						<td><div><form name="payOneUserTaskForm<%=row.UID%>" id="payOneUserTaskForm<%=row.UID%>"
						action="" method='POST'>
									<input type='hidden' name='packageid' id="packageid<%=row.UID%>" value="<%=row.UID%>" /> <input
									onclick="payOneTask(this)" type="button" value="支付包裹"
									name="updateOne"
			...
			<div style="text-align: right; margin-right: 0px">
				...
				<h4>
					<a
						href='allCurrentPayingTasksForUser.jsp?PageNumber=<%=prevpage%>'>PrevPage</a><a>
					</a><a
						href='allCurrentPayingTasksForUser.jsp?PageNumber=<%=nextpage%>'>NextPage</a>
				</h4>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function unTakenTasks() {
			location.href = "allUnPaidPackForCompany.jsp";
		}
