...
<body>
	...
	<div id="wrapper">
		<%
			...
			List<EbizPackage> shippedPackages = EbizSql.getInstance().readAllCheckedPackagesForUser(user, company,offset,
					pagesize);
		%>
		<%
		...
		<div id="wrapperContent">
			<div align="center" style="padding-bottom: 1px; padding-top: 10px;">
				<h3 style="margin-top: 5px;">所有完成的对单任务(All Finished Checking
					Tasks)</h3>
			</div>
			...
			<div id="threeBox">
				...
				<div class="alignright">
					...
					<h5>
						...
						<input onclick="checkingTasks()" type="button" value="已领取任务"
							name="updateAll"
							style="margin-top: 0px; width: 90px; font-size: 12px; height: 30px; background-color: #ffcebf">
					</h5>
				</div>
					...
					<tr>
						...
						<td
							style='font-size: 12px;line-height:14px;
												<%if (row.getStatus().equals(EbizPackageStatusEnum.Complete.getColumnName())) {%>
						background-color:#a1ff64
						<%}%><%if (row.getStatus().contains("UnMatch")) {%>
						...
						<%if (row.getStatus().contains("UnMatch")) {%>
							...
							<div style="float:left;"><form action='editPackageByChecker.jsp' method='POST'>
								<input type='hidden' name='packageid' value="<%=row.UID%>" /><input
									type='submit' style='width: 55px; height:25px;font-size: 13px' name='updateButton'
									value='Update' />
							</form></div>
							<%}%>
			...
			<div style="text-align: right; margin-right: 0px">
				...
				<h4>
					<a
						href='allCheckedPackForUser.jsp?PageNumber=<%=prevpage%>'>PrevPage</a><a>
					</a><a
						href='allCheckedPackForUser.jsp?PageNumber=<%=nextpage%>'>NextPage</a>
				</h4>
			</div>
		</div>
	</div>
